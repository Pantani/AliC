...
CURLcode
curl_fetch_url(CURL *ch, const char *url, struct curl_fetch_st *fetch)
{
...
json_object *
callRPC(const char *method, json_object *params, int id)
{
    ...
    rcode = curl_fetch_url(ch, kEnvironment, cf);
    ...
    if (rcode != CURLE_OK || cf->size < 1)
    {
        ...
        fprintf(stderr, "ERROR: Failed to fetch url (%s) - curl said: %s",
                kEnvironment, curl_easy_strerror(rcode));
        /* return error */
        return json;
    }
